...
//-----------------------获得数据块长度---------------------//
WORD  WavFile::GetDataBlock()
{
	return HT.Channels*HT.sample_bits / 8;
}
WORD  WavFile::GetFormatType()
{
	return HT.format_type;
}
DWORD WavFile::GetTransSpeed()
{
	return  HT.Channels*HT.sample_bits*HT.frequency / 8;
}
//-----------------------保存成PCM文件存储-------------------//
